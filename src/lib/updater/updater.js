...
const packageName = process.env.UPDATER_PKG_NAME || "agent-route-studio";
...
function getDataDir() {
  if (process.env.AGENT_ROUTE_HOME) return process.env.AGENT_ROUTE_HOME;
  if (process.env.DATA_DIR) return process.env.DATA_DIR;
  if (process.platform === "win32") {
    return path.join(process.env.APPDATA || path.join(os.homedir(), "AppData", "Roaming"), "agent-route-studio");
  }
  return path.join(os.homedir(), ".agent-route-studio");
}
...
// Wait until app port is listening (server alive again), then open AgentRoute Studio
async function waitForAppAndOpenBrowser() {
  const deadline = Date.now() + 30000;
  while (Date.now() < deadline) {
    const busy = await isAppPortBusy();
    if (busy) {
      openBrowser(`http://localhost:${appPort}/agent-route`);
      pushLog(`[updater] app ready, opened AgentRoute Studio`);
      return;
    }
    await sleep(1000);
  }
