...
export default function ProviderDetailPage({ providerId: providerIdOverride, embedded = false, onBack }) {
  ...
  if (!providerInfo) {
    return (
      <div className="text-center py-20">
        ...
        {embedded ? (
          <button type="button" onClick={onBack} className="text-primary mt-4 inline-block">
            Back to Providers
          </button>
        ) : (
  ...
  return (
    <div className="dashboard-provider-root flex min-w-0 flex-col gap-6 px-1 sm:gap-8 sm:px-0">
      {/* Header */}
      <div className="min-w-0">
        {embedded ? (
          <button
            type="button"
            onClick={onBack}
            className="sticky top-0 z-20 mb-4 inline-flex items-center gap-1 rounded-lg border border-border bg-surface/95 px-3 py-2 text-sm font-medium text-text-muted shadow-sm backdrop-blur transition-colors hover:border-primary/40 hover:text-primary"
          >
        ...
        <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <div className="min-w-0">
            ...
            <p className="text-xs text-text-muted">
              {isCompatible
                ? "手动添加模型，或从已连接供应商的 /models 端点导入。"
                : "点击模型可复制完整模型名，也可以添加自定义模型。"}
            </p>
          </div>
          <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center sm:justify-end">
            {!isCompatible && (
              <Button size="sm" icon="add" onClick={() => setShowAddCustomModel(true)} className="w-full sm:w-auto">
                添加模型
              </Button>
            )}
            {!isCompatible &&
