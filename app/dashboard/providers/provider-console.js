...
export default function ProvidersPage({ embedded = false, onOpenProvider }) {
  ...
  }, [embedded, registerSearch, unregisterSearch]);
  ...
  const oauthEntries = Object.entries(OAUTH_PROVIDERS).filter(([, info]) => !info.hidden && matchSearch(info.name));
  const freeEntries = Object.entries(FREE_PROVIDERS).filter(([, info]) => !info.hidden && matchSearch(info.name));
  ...
  const apikeyEntries = sortByPriority(
    Object.entries(APIKEY_PROVIDERS).filter(
      ([, info]) => !info.hidden && (info.serviceKinds ?? ["llm"]).includes("llm") && matchSearch(info.name)
    ),
    "apikey"
  );
  const isApikeySearching = !!searchQuery.trim();
  const visibleApikeyEntries =
    isApikeySearching || showAllApikey ? apikeyEntries : apikeyEntries.slice(0, APIKEY_INITIAL_VISIBLE);
  ...
  return (
    <div className="dashboard-provider-root flex min-w-0 flex-col gap-6 px-1 sm:px-0">
      {!hasAnyResult && (
...
function ProviderCard({ providerId, provider, stats, authType, onToggle, onOpen }) {
  ...
  const openProvider = () => {
    if (onOpen) onOpen(providerId);
    else router.push(`/dashboard/providers/${providerId}`);
  };
...
function ApiKeyProviderCard({ providerId, provider, stats, authType, onToggle, onOpen }) {
  const router = useRouter();
  const { connected, error, errorCode, errorTime, allDisabled } = stats;
    ...
    <Modal isOpen={isOpen} title="Add OpenAI Compatible" onClose={onClose}>
      <div className="flex flex-col gap-4">
        ...
        <Input
          ...
          hint="Use the provider base URL (ending in /v1) for Agent internal model calls."
        />
        <Input
        ...
        <Input
          ...
          hint="If provider lacks /models endpoint, enter a model ID to validate via chat/completions."
        />
