...
export default function OAuthModal({ isOpen, provider, providerInfo, onSuccess, onClose, oauthMeta, idcConfig }) {
  ...
  async function readOAuthResponse(res) {
    const text = await res.text();
    if (!text) {
      return {
        error: res.ok ? "" : `OAuth 服务返回了空响应（HTTP ${res.status}）`
      };
    }
    try {
      ...
      return {
        error: `OAuth 服务返回了非 JSON 响应（HTTP ${res.status}）：${text.slice(0, 160)}`
      };
    }
  }
  ...
  const exchangeTokens = useCallback(
    async (code, state) => {
      ...
      try {
        ...
        const data = await readOAuthResponse(res);
        if (!res.ok) throw new Error(data.error || data.message || `OAuth 连接失败（HTTP ${res.status}）`);
  ...
  const startOAuthFlow = useCallback(async () => {
    ...
    try {
      ...
      // If a provider needs a client secret, keep it out of the local URL/query string.
      let res;
      if (oauthMeta?.clientSecret || oauthMeta?.client_secret) {
        res = await fetch(`/api/oauth/${provider}/authorize`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            redirectUri,
        ...
        res = await fetch(authorizeUrl.toString());
      }
      const data = await readOAuthResponse(res);
      if (!res.ok) throw new Error(data.error || data.message || `OAuth 连接失败（HTTP ${res.status}）`);
      ...
      if (provider === "codex") {
        try {
          ...
          const proxyData = await readOAuthResponse(proxyRes);
