...
1. **Agent 内部模型服务**（`src/core`）：负责读取模型 API 设置、上游调用和响应格式兼容。只供 commander、planner、worker、verifier、reviewer、finalizer 等内部角色使用，不对外提供模型代理产品。
...
app/
  agent-route/                 # 前端工作台（控制台 + 运行监控）
    chat/                      # 聊天式运行视图
  dashboard/                   # legacy provider 路由重定向
  api/
    agent-route/ui-stream/     # 目标运行主入口（UIMessage 流）
    agent-route/run/           # action API（旧 SSE 目标流已禁用，返回 410）
    mcp/                       # 对外 MCP Streamable HTTP 端点
    model-apis/                # 模型 API 设置
    providers/ provider-nodes/ # legacy provider API，返回 410
    oauth/[provider]/[action]/ # legacy OAuth API，返回 410
...
src/
  core/        # Agent 内部模型服务（router / model API settings / model catalog）
  agent/       # 目标驱动智能体系统
  config/      # 默认配置、策略、加载器
...
位置：`src/core/router`、`src/core/model-api-settings.js`、`src/config/models`。

职责：

- 接收 Agent 内部模型请求，路由到上游模型服务。
- 支持 OpenAI Chat Completions 兼容调用；Claude 使用 Anthropic Messages 并在内部转换为 OpenAI Chat Completion 形状。
- 处理模型 API 配置、模型前缀、模型别名和响应格式兼容。
- 支持的 provider 固定为 OpenAI、Claude、Gemini、Grok、DeepSeek、Qwen、GLM、Kimi；OpenRouter、OAuth provider、自定义 provider node 和通用 upstream env 回退已移除。

不负责：goal/task 生命周期、记忆、风险、验证、预算、工具执行。

Agent 的 planner、模型 worker、verifier、reviewer 与 finalizer 统一通过内部 `/v1/chat/completions` function calling 提交结构化结果：运行时为当前阶段强制指定唯一 `tools` / `tool_choice`，并只接受对应 `tool_calls[].function.arguments`。文本 JSON、旧 `function_call` 与 `response_format` 路径不作为回退。MCP worker 的 JSON 结果仍是工具协议 payload，不属于模型输出通路。
...
src/agent/orchestrator/runtime.js        # LangGraph 节点使用的运行时 session
src/agent/orchestrator/langgraph-runner.js # LangGraph 状态机
```

`POST /api/agent-route/ui-stream` → `handleAgentRouteUiStream` → `langGraphRunner.runAgentRouteLangGraph`，目标执行由 LangGraph `StateGraph` 驱动：
...
`runtime.js` 不再暴露旧的整段目标循环；它只创建 LangGraph 节点调用的 runtime session。一次目标运行大致如下：

```text
Goal
 └─ goalSetup.createRunState   建运行状态、载入记忆、准备 budget / strategy
     └─ plan_or_resume          commander 生成初始任务（resume 时从 store 读回）
         └─ begin_iteration     预算 / 策略停止判定
             └─ select_ready_task  只从 dependency graph 选择 ready task
                 └─ run_ready_task:
             ...
             └─ review_iteration   复盘：给最终答案 / 追加任务 / 继续
         └─ finalize_goal:
...
src/tools/files      # 文件读写、目录枚举、文件发现、哈希、临时文件
...
内部 worker MCP 边界位于 `src/agent/mcp/client.js`，仅在进程内使用 `InMemoryTransport`，把 worker dispatcher 到 document / web / files / browser / codex-cli worker 的调用统一为标准 MCP `tools/list`、`tools/call`、`resources/read` 形态；这些 worker tools 不注册到公开 `/api/mcp`。本地文件/目录只读取证使用 `agentroute.worker.files`，不经过 codex-cli 代读。内部 worker 不提供回退到原直接调用路径的配置；协议或 schema 错误直接暴露为调用失败，工具执行失败则作为 MCP tool result 返回，保留原 worker result 结构供 retry、verification 和 observability 使用。
